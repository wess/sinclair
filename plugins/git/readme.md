...
A live Git side panel built on Sinclair's plugin API. Shows the current branch,
...
- Install: copy this folder into `~/.config/sinclair/plugins/git`, then open the
...
This is an IPC plugin: Sinclair invokes `bun run plugin.ts` per render/action,
