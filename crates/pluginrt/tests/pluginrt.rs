...
fn git_plugin_runs_entirely_through_host_process() {
  let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../plugins/git/plugin.wasm");
...
fn bundled_screentools_greps_the_screen() {
  let path =
    std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../plugins/screentools/plugin.wasm");
