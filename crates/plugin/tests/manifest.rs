...
"#;
    ...
    assert!(diags
        .iter()
        .any(|d| d.message.contains("[runtime] requires")));
}
...
"#,
    ...
    assert_eq!(
        ...
        TriggerAction::Notify {
            text: "A command failed".into()
        }
    );
    assert_eq!(
        t[1].action,
        TriggerAction::Run {
            text: "direnv reload".into(),
            target: TriggerTarget::Background
        }
    );
    assert_eq!(
        t[2].action,
...
fn trigger_unknown_event_is_dropped() {
    ...
    assert!(diags
        .iter()
        .any(|d| d.message.contains("unknown trigger event")));
}

#[test]
fn trigger_needs_exactly_one_action() {
    let (none_plugin, none_diags) = parse(path(), "id = \"w\"\n[[trigger]]\non = \"bell\"\n");
    assert!(none_plugin.unwrap().triggers.is_empty());
    assert!(none_diags
        .iter()
        .any(|d| d.message.contains("exactly one action")));
...
fn wasm_type_is_optional_and_a_process_runtime_is_diagnosed() {
    ...
    assert!(plugin.unwrap().runtime.is_none());
    assert!(
        diags.iter().any(|d| d.message.contains("removed")),
        "{diags:?}"
    );
    ...
    assert!(
        diags
            .iter()
            .any(|d| d.message.contains("no longer supported")),
        "{diags:?}"
    );
}
...
fn wasm_module_path_may_not_escape_the_plugin_dir() {
    for module in ["/etc/passwd", "../../evil.wasm", "ok/../../evil.wasm"] {
        ...
        assert!(
            diags
                .iter()
                .any(|d| d.message.contains("inside the plugin folder")),
            "{module}: {diags:?}"
        );
    }
...
fn bundled_manifests_parse_cleanly() {
    ...
    assert!(
        checked >= 5,
        "expected the bundled plugins, checked {checked}"
    );
}
