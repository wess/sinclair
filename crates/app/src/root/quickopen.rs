...
impl WorkspaceView {
    ...
    pub(crate) fn open_quickopen(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        items.push(("Manage Plugins".to_string(), None, Action::ManagePlugins));
        self.open_spotlight(items, window, cx);
    }
}
...
const SYMBOLS: &[(&str, &str)] = &[
    ...
    ("\u{2764}\u{fe0f}", "heart red"),
    ("\u{1f9e0}", "brain"),
    ("\u{1f916}", "robot"),
    ("\u{1f9d1}\u{200d}\u{1f4bb}", "developer"),
    ("\u{1f4c1}", "folder"),
    ("\u{1f4c4}", "document page"),
    ("\u{1f4cc}", "pin"),
    ("\u{1f4dd}", "memo note"),
    ("\u{1f512}", "lock secure"),
    ("\u{1f513}", "unlock"),
    ("\u{1f511}", "key"),
    ("\u{2699}\u{fe0f}", "gear settings"),
    ("\u{1f527}", "wrench tool"),
    ("\u{1f4e6}", "package box"),
    ("\u{1f550}", "clock time"),
    ("\u{2b50}", "star"),
    ...
    ("\u{1f4c8}", "chart up trend"),
    ("\u{1f4c9}", "chart down"),
    ("\u{1f6a7}", "construction wip"),
    ("\u{2192}", "arrow right"),
    ("\u{2190}", "arrow left"),
    ("\u{2191}", "arrow up"),
    ("\u{2193}", "arrow down"),
