  ...
  pub fn mcp_dispatch(
    ...
    match op {
      ...
      "update_probe" => {
        ...
        let (state, latest, err) = match updater::check(crate::updateui::current(), &detected) {
          Ok(updater::Check::Ready(r)) => ("ready", r.version, String::new()),
          Ok(updater::Check::Pending(v)) => ("pending", v, String::new()),
          Ok(updater::Check::UpToDate) => ("up-to-date", String::new(), String::new()),
      ...
      "action" => {
        ...
        let action = Action::parse(spec).map_err(|e| format!("bad action `{spec}`: {e}"))?;
      ...
      "list_tabs" => {
        ...
        let tabs = self
          ...
          .enumerate()
          .map(
            |(index, title)| json!({ "index": index, "title": title, "active": index == active }),
          )
          .collect::<Vec<_>>();
