...
impl WorkspaceView {
  ...
  pub(crate) fn peek_hover_band(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
    ...
    Some(
      div()
        ...
        .h(px(TAB_HEIGHT))
        .on_hover(
          cx.listener(|this, hovered: &bool, _window, cx| this.peek_hover_bar(*hovered, cx)),
        )
        .into_any_element(),
    )
  }
  ...
  pub(crate) fn peek_strip(
    ...
    Some(
      div()
        ...
        .occlude()
        .on_hover(
          cx.listener(|this, hovered: &bool, _window, cx| this.peek_hover_strip(*hovered, cx)),
        )
  ...
  fn peek_card(
    ...
    let width = match &session {
      Some(session) => {
        (body_h * libsinclair::preview::aspect(session, self.cell)).clamp(CARD_MIN_W, CARD_MAX_W)
      }
      None => CARD_PLAIN_W,
    };
