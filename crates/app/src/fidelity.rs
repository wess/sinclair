...
impl GpuiRaster {
  ...
  pub fn new(base: Font, font_size: f32, scale: f32) -> GpuiRaster {
    let platform: Arc<dyn PlatformTextSystem> = gpui_platform::current_platform(true).text_system();
...
impl Raster for GpuiRaster {
  ...
  fn frame(&mut self, term: &Terminal, colors: &Colors, out: &mut Rgba) {
    ...
    for row in 0..rows {
      ...
      let flush =
        |out: &mut Rgba, this: &GpuiRaster, span: &mut Option<(usize, String, CellFlags, Rgb)>| {
