...
pub fn explain(text: &str) -> String {
  ...
  if lower.contains("permission denied") || lower.contains("eacces") {
    ...
    lines.push("Check spelling, PATH, installed tools, and relative paths from this pane's cwd.");
    ...
    lines.push(
      "Start with the first error line, then check the command arguments and local environment.",
    );
  } else {
