...
Sinclair's Windows artifacts, produced by `scripts/windows.ps1` (mirrors
...
sinclair until an Authenticode certificate is wired in.

## Artifacts

`scripts/windows.ps1 [-Arch x86_64]` writes to `dist/windows/`:

- **`sinclair-<version>-windows-<arch>.zip`** — portable build: `sinclair.exe`, the
  `notes.exe` sidecar, and bundled plugins. Unzip and run `sinclair.exe`.
- **`sinclair-<version>-windows-<arch>.msi`** — WiX v4 installer (per-machine,
...
- **Scoop** (`scoop/sinclair.json`): the release workflow rewrites the version,
  URL, `extract_dir`, and SHA-256 and commits it back. Install once published:
  `scoop install https://raw.githubusercontent.com/wess/sinclair/main/packaging/scoop/sinclair.json`
- **Chocolatey** (`chocolatey/sinclair.nuspec` + `tools/chocolateyinstall.ps1`):
